...
class LocalDateTimeTest {
    ...
    @Test fun one_day_gap_for_a_year() {
        LocalDateTime(year = 2023, monthNumber = 1, dayOfMonth = 1, hour = 0, minute = 0)
            .nextMatchSequence(atSeconds = 0..0, atMinutes = 0..0, atHours = 0..0)
            .takeWhile { it.year < 2024 }
            .toList()
            .also { assertEquals(expected = 365, actual = it.size) }
            .windowed(2) { (first, second) ->
                assertEquals(
                    expected = DateTimePeriod(days = 1),
                    actual = first.toInstant(TimeZone.UTC).periodUntil(
                        other = second.toInstant(TimeZone.UTC),
                        timeZone = TimeZone.UTC
                    ),
                    message = "The difference beteween $first and $second is not one day."
                )
            }
    }
}
